...
class GameplayActivity : AppCompatActivity(),
            ...
            buttonA.setOnClickListener {
                if (canChangeMap) {
                    when (targetDestination) {
                        ...
                        "biblioteca" -> startBibliotecaActivity()
                        "salida" -> startSalidaActivity()
    ...
    private fun startSalidaActivity() {
        val intent = Intent(this, Salida::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
            putExtra("INITIAL_POSITION", Pair(29, 2))
            putExtra("PREVIOUS_POSITION", gameState.playerPosition) // Guarda la posición actual
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
        }
        startActivity(intent)
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        ...
        when {
            ...
            position.first == 9 && position.second == 14 -> {
                canChangeMap = true
                targetDestination = "biblioteca"
                runOnUiThread {
                    Toast.makeText(this, "Presiona A para entrar a la biblioteca", Toast.LENGTH_SHORT).show()
                }
            }

            position.first == 38 && position.second == 32 -> {
                canChangeMap = true
                targetDestination = "salida"
                runOnUiThread {
